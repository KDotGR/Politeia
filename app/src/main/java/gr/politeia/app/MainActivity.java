...
public final class MainActivity extends Activity {
 private static final int BG=0xFFF5F7FA,INK=0xFF152A44,MUTED=0xFF526478,TEAL=0xFF008578,LINE=0xFFE0E6EE;
 ...
 private Button button(String label,String tag,Runnable action,boolean primary){Button b=new Button(this);b.setText(label);b.setTag(tag);b.setAllCaps(false);b.setTextSize(14);b.setTypeface(Typeface.DEFAULT,Typeface.BOLD);b.setTextColor(primary?Color.WHITE:INK);b.setPadding(dp(12),dp(9),dp(12),dp(9));b.setMinHeight(dp(50));b.setMinimumHeight(dp(50));b.setStateListAnimator(null);b.setBackground(new RippleDrawable(android.content.res.ColorStateList.valueOf(0x22008578),bg(primary?TEAL:0xFFEAF0F6,12),null));b.setSoundEffectsEnabled(false);b.setOnClickListener(v->{
   v.performHapticFeedback(HapticFeedbackConstants.VIRTUAL_KEY);
   if(!Arrays.asList("next-step","previous-step","calculate","edit","sound").contains(tag))sound(false);
   action.run();});return b;}
 private LinearLayout card(LinearLayout parent,String heading){LinearLayout box=column();box.setPadding(dp(20),dp(18),dp(20),dp(20));box.setBackground(bg(Color.WHITE,20));LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(-1,-2);p.setMargins(0,dp(8),0,dp(8));parent.addView(box,p);if(heading!=null){box.addView(text(heading,19,INK,true));space(box,12);}return box;}
 ...
 private void sourceButton(LinearLayout parent,String label,String tag,String mode){
  ...
  choice.setSelected(sourceMode.equals(mode));if(choice.isSelected()){choice.setTextColor(TEAL);choice.setBackground(bg(0xFFDDF3EE,12));}parent.addView(choice);
 ...
 private void renderInputs(){
  ...
  error=text("",14,0xFFB4233A,true);error.setTag("error");error.setVisibility(View.GONE);inputs.addView(error);space(inputs,14);inputs.addView(button(t("Calculate seats →","Υπολογισμός εδρών →"),"calculate",this::calculate,true));
 }
 ...
 private void renderResults(){stepButton(body,t("← Previous","← Προηγούμενο"),"previous-step",()->goToStep(4),false);space(body,20);LinearLayout results=column();results.setTag("share-results");body.addView(results);results.addView(text(t("politeia · THE SEAT PICTURE","πολιτεία · Η ΕΙΚΟΝΑ ΤΩΝ ΕΔΡΩΝ"),11,TEAL,true));space(results,6);pageTitle(results,t("Your results, explained.","Τα αποτελέσματα, με εξήγηση."));description(results,title()+" · "+ruleName());
